...
typedef void(^LYFAlbumViewSelectAction)(QVAlbumModel *albumModel);

@interface QVAlbumView()
...
/// 相册列表
@property (nonatomic, strong) QVAlbumTableView *tableView;
/// 列表的高度
...
+(void)showAlbumView:(NSMutableArray<QVAlbumModel *> *)assetCollectionList navigationBarMaxY:(CGFloat)navigationBarMaxY complete:(void(^)(QVAlbumModel *albumModel))complete {
    QVAlbumView *albumView = [[QVAlbumView alloc] init];
...
-(void)setAssetCollectionList:(NSMutableArray<QVAlbumModel *> *)assetCollectionList {
    ...
    __weak typeof(self) weakSelf = self;
    self.tableView.selectAction = ^(QVAlbumModel *albumModel) {
...
#pragma mark - Get方法
-(QVAlbumTableView *)tableView {
    if (!_tableView) {
        _tableView = [[QVAlbumTableView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, kScreenHeight) style:UITableViewStylePlain];
