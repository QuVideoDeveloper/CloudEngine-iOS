...
#import "QVAlbumModel.h"
#import "QVPhotoManger.h"
#import "QVAlbumView.h"
#import "QVPhotoModel.h"

#define kScreenWidth  [[UIScreen mainScreen] bounds].size.width
#define kScreenHeight [[UIScreen mainScreen] bounds].size.height

@interface QVAlbumViewController () <UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout>
...
@implementation QVAlbumViewController
...
-(void)setupViewController {
    ...
    [QVPhotoManger standardPhotoManger].choiceCountChange = ^(NSInteger choiceCount) {
        ...
        if (choiceCount == 0) {
            ...
            [weakSelf.confirmButton setTitle:[NSString stringWithFormat:@"确定%ld/%ld", [QVPhotoManger standardPhotoManger].choiceCount, [QVPhotoManger standardPhotoManger].maxCount] forState:UIControlStateNormal];
...
-(void)getThumbnailImages {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        ...
        for (PHAssetCollection *collection in cameraRolls) {
            QVAlbumModel *model = [[QVAlbumModel alloc] init];
...
#pragma mark - Set方法
-(void)setAlbumModel:(QVAlbumModel *)albumModel {
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    QVAlbumCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:albumCollectionViewCell forIndexPath:indexPath];
    ...
    cell.selectPhotoAction = ^(PHAsset *asset) {
        ...
        if ([weakSelf.albumModel.selectRows containsObject:@(indexPath.row)]) {
            ...
            [QVPhotoManger standardPhotoManger].choiceCount--;
            
            isReloadCollectionView = [QVPhotoManger standardPhotoManger].choiceCount == 9;
        } else {
            ...
            [QVPhotoManger standardPhotoManger].choiceCount++;
            isReloadCollectionView = [QVPhotoManger standardPhotoManger].choiceCount == 10;
        }
...
-(void)showAlbum:(UIButton *)button {
    ...
    [QVAlbumView showAlbumView:self.assetCollectionList navigationBarMaxY:CGRectGetMaxY(self.navigationController.navigationBar.frame) complete:^(QVAlbumModel *albumModel) {
        if (albumModel) {
...
-(void)confirmAction:(UIButton *)button {
    if ([QVPhotoManger standardPhotoManger].choiceCount > 0) {
        button.enabled = NO;
        NSMutableArray<QVPhotoModel *> *photoList = [NSMutableArray array];
        
        for (QVAlbumModel *albumModel in self.assetCollectionList) {
            for (NSNumber *row in albumModel.selectRows) {
                if (row.integerValue < albumModel.assets.count) {
                    QVPhotoModel *photoModel = [[QVPhotoModel alloc] init];
        ...
        [QVPhotoManger standardPhotoManger].photoModelList = photoList;
...
-(UICollectionView *)albumCollectionView {
    if (!_albumCollectionView) {
        ...
        [_albumCollectionView registerClass:[QVAlbumCollectionViewCell class] forCellWithReuseIdentifier:albumCollectionViewCell];
